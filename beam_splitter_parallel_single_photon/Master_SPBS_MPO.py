...
class MasterMPO:
    def __init__(self, num_gpu_ranks, n, m, d, loss, chi, errtol = 10 ** (-6), PS = None):
    ...
    def MPOInitialization(self):
        ...
        chi = self.chi; init_chi = self.chi; d = self.d; K = self.m
    ...
    def MPOInitialization1(self):
        ...
        chi = self.chi; d = self.d; init_chi = self.chi
    ...
    def MasterRequest(self, seed, l, r, target_rank):
        ...
        # else:
        chi = self.chi
        new_charge = self.d * np.ones([chi, 2], dtype='int32')
        new_Lambda = np.zeros(chi, dtype='float32')
        Gamma0Out = np.zeros([chi, chi], dtype='complex64')
        ...
        self.requests[l] = [new_charge_req, new_Lambda_req, Gamma0Out_req, Gamma1Out_req, U_time_req, svd_time_req, theta_time_req, align_init_time_req, align_info_time_req, index_time_req, copy_time_req, align_time_req, before_loop_other_time_req, segment1_time_req, segment2_time_req, segment3_time_req, comm_time_req]
        ...
        for k in range(self.n - 1):
            ...
            if k < self.n / 2:
                ...
                for i in range(2 * self.n - 2 * k - 2):
                    ...
                    seed = np.random.randint(0, 13579)
                    self.seeds[first_layer + i, self.n//2-1-(i+1)//2] = seed
                    # np.random.seed(seed)
                    # print(seed)
                    l -= 1    
    ...
    def LayerUpdate(self, k):
        ...
        for i, l in enumerate(range(k % 2, self.n - 1, 2)):
            ...
            self.MasterRequest(seed, l, reflectivity, target_rank)
