...
class MPS:
    ...
    def MPSInitialization(self):
        self.MPSInitializationOriginal()
        self.MPSInitializationHost()
        self.MPSInitializationDevice()

    def MPSInitializationOriginal(self):
    ...
    def MPSInitializationHost(self):
        self.hGamma = np.zeros([self.n, self.chi, self.chi], dtype = 'complex64'); # alpha, alpha, I, modes
        self.hLambda = np.zeros([self.n - 1, self.chi]); # alpha, modes - 1
        self.hLambda_edge = np.ones(self.chi, dtype=float) # edge lambda don't exists and are ones
        self.hcharge = np.zeros([self.n + 1, self.chi]);
        ...
        for i in range(self.m):
            self.hcharge[i, :] = self.m - i

    def MPSInitializationDevice(self):
    ...
    def MPStwoqubitUpdate(self, l, r):
        ...
        Lambda1 = np.sort(self.MPStwoqubitUpdateDevice(l, r, seed))
        Lambda2 = np.sort(self.MPStwoqubitUpdateHost(l, r, seed))

        if not np.allclose(Lambda1, Lambda2, atol=0.00001):
            print("Disagree", Lambda1, Lambda2)
    ...
    def MPStwoqubitUpdateHost(self, l, r, seed):
        
        np.random.seed(seed)
        UnitaryMPS = Rand_BS_MPS_np(self.d, r)
        #UnitaryMPS = Rand_BS_MPS(self.d, r);
        ...
        if l == 0:
            location = left
            Lambda_l = self.hLambda_edge
            Lambda_c = self.hLambda[l]
            Lambda_r = self.hLambda[l+1]
        elif l == self.n - 2:
            location = right
            Lambda_l = self.hLambda[l-1]
            Lambda_c = self.hLambda[l]
            Lambda_r = self.hLambda_edge
        else:
            location = center
            Lambda_l = self.hLambda[l-1]
            Lambda_c = self.hLambda[l]
        ...
        Gamma_lc = self.hGamma[l]
        Gamma_cr = self.hGamma[l+1]
        # charge of corresponding index (bond charge left/center/right)
        bc_l = self.hcharge[l]
        bc_c = self.hcharge[l+1]
        bc_r = self.hcharge[l+2]
        ...
        for j in range(self.d):
            idx_L.append(np.array(list(set.intersection(set(np.nonzero(bc_l == j)[0]), set(np.nonzero(Lambda_l > self.errtol)[0]))), dtype = 'int32')) #left hand side charge
            idx_C.append(np.array(list(set.intersection(set(np.nonzero(bc_c == j)[0]), set(np.nonzero(Lambda_c > self.errtol)[0]))), dtype = 'int32')) #left hand side charge
            idx_R.append(np.array(list(set.intersection(set(np.nonzero(bc_r == j)[0]), set(np.nonzero(Lambda_r > self.errtol)[0]))), dtype = 'int32')) #left hand side charge
        ...
        for tau in range(self.d):
            ...
            l_bond = np.array(list(itertools.chain(*[idx_L[i] for i in l_charge])), dtype=int) # bond index corresponding to some charges
            c_bond = np.array(list(itertools.chain(*[idx_C[i] for i in c_charge])), dtype=int)
            r_bond = np.array(list(itertools.chain(*[idx_R[i] for i in r_charge])), dtype=int)
            l_bond_array.append(l_bond)
            r_bond_array.append(r_bond)
            ...
            idx_l = [idx_L[i] if i in l_charge else np.array([]) for i in range(self.d)]
            idx_c = [idx_C[i] if i in c_charge else np.array([]) for i in range(self.d)]
            idx_r = [idx_R[i] if i in r_charge else np.array([]) for i in range(self.d)]

            C = np.zeros([len_l, len_r], dtype = 'complex64');
            update_MPS_np(tau, self.d, idx_l, idx_c, idx_r, C, UnitaryMPS, Lambda_l, Gamma_lc, Lambda_c, Gamma_cr, Lambda_r)
            # temp_l = 0;
            # for ch_l in l_charge:
            #     if len(idx_L[ch_l]) == 0:
            #         continue
            ...
            #         temp_r += len(idx_R[ch_r]);
            #         for ch_c in range(ch_r, ch_l + 1):
            #             C[temp_l - len(idx_L[ch_l]):temp_l, temp_r - len(idx_R[ch_r]):temp_r] += UnitaryMPS[ch_l - tau, tau - ch_r, ch_l - ch_c, ch_c - ch_r] * np.multiply(np.matmul(np.multiply(np.multiply(Lambda_l[idx_L[ch_l]].reshape(-1, 1), Gamma_lc[idx_L[ch_l].reshape(-1, 1), idx_C[ch_c].reshape(1, -1)]), Lambda_c[idx_C[ch_c]].reshape(1, -1)), Gamma_cr[idx_C[ch_c].reshape(-1, 1), idx_R[ch_r].reshape(1, -1)]), Lambda_r[idx_R[ch_r]].reshape(1, -1))
            self.update_count += 1
        ...
        temp = np.zeros([self.chi]);
        temp[:num_lambda] = Lambda_temp[idx]
        #print('cpu: ', temp, Lambda_temp[idx], temp[:num_lambda])
        self.hLambda[l, :] = temp
        self.hcharge[l+1, :num_lambda] = new_charge[idx]
        ...
        if location == left:
            self.hGamma[0, 0, :] = Gamma1Out[0, :]; self.hGamma[1, :, :] = Gamma2Out;
        elif location == right:
            self.hGamma[self.n - 2, :, :] = Gamma1Out; self.hGamma[self.n - 1, :, 0] = Gamma2Out[:, 0];
        else:
            self.hGamma[l, :, :] = Gamma1Out; self.hGamma[l + 1, :, :] = Gamma2Out;
